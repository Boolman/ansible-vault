...
INTR_PATH=${INTR_PATH:-intermediate_ca}

COMPANY="${COMPANY:-DCC Access AB.}"
DOMAIN="${DOMAIN:-int.units.cloud}"
UNDER_DOM=${DOMAIN//\./_}
...
echo ${ROOT_PATH}
echo ${COMPANY}
http POST ${VAULT_ADDR}/v1/$ROOT_PATH/root/generate/exported X-Vault-Token:$VAULT_TOKEN common_name="${COMPANY} Root CA" ttl="87600h" | jq -r . > root.json
...
# Set the max TTL for ${DOMAIN} certs to 10 year
echo "Tuning intermediate CA"
vault mount-tune -max-lease-ttl=87600h ${INTR_PATH}

# Generate CSR for ${DOMAIN} to be signed by the root CA, the key is stored
# internally to vault
echo "Generating intermediate CSR"
http POST ${VAULT_ADDR}/v1/${INTR_PATH}/intermediate/generate/exported X-Vault-Token:$VAULT_TOKEN common_name=${DOMAIN} | jq -r .data > intermediate.json
cat intermediate.json | jq -r .csr > ${UNDER_DOM}.csr
...
# TODO 
# vault read intermediate_ca/cert/ca > chain.pem
# vault read root_ca/cert/ca >> chain.pem
# vault write intermediate_ca/intermediate/set-signed ceritifcate=@chain.pem
#vault write intermediate_ca/config/urls issuing_certificates="https://vault.units.cloud:8200/v1/intermediate_ca/ca" crl_distribution_points="https://vault.units.cloud:8200/v1/intermediate_ca/crl"
